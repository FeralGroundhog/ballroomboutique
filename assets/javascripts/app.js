...
(function($) {
  ...
  $(document).ready(function() {
    ...
    var personalOpts = {
      anchors: ["top", "about", "instructor", "client-gallery", "contact"],
      navigationTooltips: ["home", "about", "instructor", "gallery", "contact"]
    };
    ...
    if(!$(event.target).closest('.top-bar-section').length) {
        if($('.top-bar-section').is(":visible")) {
            $('.top-bar-section').fadeToggle();
        }
    }
    ...
    onBinding: function() {
      ...
      var fullscreen_slide = function() {
        if ( !$ctx.hasClass('fullheight') ) {
          $('.slides', $ctx).width($(window).width());
        }
        if( $ctx.hasClass('force')) {
          $('.slides', $ctx).height($(window).height());
        } else {
          $('.slides', $ctx).height($(window).height() - $('.top-bar').height());
        }
      }
